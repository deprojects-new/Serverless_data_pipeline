...
def handle_schema_validation(df): #To validate schema
    expected_fields = [
        "event_id", "event_ts", "session_id", "method", "path", "status", 
        "bytes_sent", "response_time_ms", "referrer", "user_agent", "user_id",
        "cache_status", "cdn_edge", "db_query_time_ms", "request_id"
    ]
...
def cast_data_types(df): 
    ...
    df = df.withColumn("user_id", when(col("user_id").isNull(), lit(None)).otherwise(col("user_id")))
    df = df.withColumn("referrer", when(col("referrer").isNull(), lit("direct")).otherwise(col("referrer")))
...
def apply_data_validations(df):
    ...
    df = df.filter(col("method").isin(["GET", "POST", "PUT", "DELETE", "PATCH", "HEAD", "OPTIONS"]))
    
    # Performance validations
    df = df.filter((col("response_time_ms").isNotNull()) & (col("response_time_ms") > 0) & (col("response_time_ms") <= 30000))
    df = df.filter((col("bytes_sent").isNotNull()) & (col("bytes_sent") >= 0) & (col("bytes_sent") <= 10000000))
    
    # Data integrity validations
    df = df.filter(col("event_ts").isNotNull())
    df = df.filter((col("path").isNotNull()) & (col("path") != "") & (col("path") != "//"))
    ...
    logger.info(f"Validation: {initial_count:,} → {final_count:,} ({initial_count-final_count:,} rejected)")
    
    return df

def add_enrichment_fields(df):
    # Status indicators
    df = df.withColumn("is_client_error", when(col("status").between(400, 499), 1).otherwise(0))
    df = df.withColumn("is_server_error", when(col("status").between(500, 599), 1).otherwise(0))
    df = df.withColumn("is_success", when(col("status").between(200, 299), 1).otherwise(0))
    df = df.withColumn("is_redirect", when(col("status").between(300, 399), 1).otherwise(0))
    ...
    # Size indicators
    df = df.withColumn("is_large_response", when(col("bytes_sent") > 100000, 1).otherwise(0))
    df = df.withColumn("is_small_response", when(col("bytes_sent") < 1000, 1).otherwise(0))
...
def process_data():
    try:
        ...
        try:
            ...
            response = s3_client.list_objects_v2(Bucket=bucket, Prefix="bronze/", MaxKeys=1)
            if 'Contents' not in response:
                logger.error("No objects found in bronze folder")
                logger.error("This suggests no data has been uploaded to the bronze layer")
                return False
            else:
                logger.info(f"Bronze folder validation: Found {response.get('KeyCount', 0)} objects")
        ...
        try:
            ...
            if not log_files:
                logger.warning("No log files matching pattern logs_YYYYMMDD_HHMMSS.json found")
                return True
